...
plot_a_type<- get(results_name) %>% 
  ...
  group_by(ind_idx,stringency,adherence_quar,type,assay,quar_dur,n_tests,delay_scaling,sampling_freq) %>% 
  filter(!is.infinite(inf_start) & !is.infinite(inf_end)) %>% 
  summarise(all=sum(inf_end-inf_start),
            prop=sum(max_overlap)/all
  ) %>% 
  group_by(stringency,adherence_quar,assay,type,quar_dur,n_tests,delay_scaling,sampling_freq) %>% 
  nest() %>%
  mutate(Q    = map(.x=data,
                    ~quantile(.$prop,
                              probs = c(0.025,0.25,0.5,0.75,0.975))),
...
plot_b_type <- get(results_name) %>% 
  ...
  group_by(ind_idx,stringency,assay,type,n_tests,delay_scaling,sampling_freq) %>% 
  filter(!is.infinite(inf_start) & !is.infinite(inf_end)) %>% 
  summarise(all=sum(inf_end-inf_start),
            prop=sum(max_overlap)/all
  ) %>% 
  group_by(stringency,assay,type,n_tests,delay_scaling,sampling_freq) %>% 
  nest() %>%
  mutate(Q    = map(.x=data,
                    ~quantile(.$prop,
                              probs = c(0.025,0.25,0.5,0.75,0.975))),
...
plot_a_lfa <- get(results_name) %>% 
  ...
  group_by(ind_idx,stringency,adherence_iso,adherence_quar,sens_scaling,assay,quar_dur,n_tests,delay_scaling,sampling_freq) %>% 
  filter(!is.infinite(inf_start) & !is.infinite(inf_end)) %>% 
  summarise(all=sum(inf_end-inf_start),
            prop=sum(max_overlap)/all
  ) %>% 
  group_by(stringency,adherence_iso,adherence_quar,sens_scaling,assay,quar_dur,n_tests,delay_scaling,sampling_freq) %>% 
  nest() %>%
  mutate(Q    = map(.x=data,
                    ~quantile(.$prop,
                              probs = c(0.025,0.25,0.5,0.75,0.975))),
  ...
  labs(x=expression("Quarantine required until"~italic("n")~"days have passed since exposure"),
       y="Transmission potential averted")+
  facet_nested(nest_line=T,
               ~sens_scaling, labeller = labeller(
                 type = capitalize,
                 delay_scaling = delay_scaling_labeller,
...
plot_b_lfa <- get(results_name) %>% 
  ...
  group_by(ind_idx,stringency,adherence_iso,assay,sens_scaling,n_tests,delay_scaling,sampling_freq) %>% 
  filter(!is.infinite(inf_start) & !is.infinite(inf_end)) %>% 
  summarise(all=sum(inf_end-inf_start),
            prop=sum(max_overlap)/all
  ) %>% 
  group_by(stringency,adherence_iso,assay,n_tests,sens_scaling,delay_scaling,sampling_freq) %>% 
  nest() %>%
  mutate(Q    = map(.x=data,
                    ~quantile(.$prop,
                              probs = c(0.025,0.25,0.5,0.75,0.975))),
