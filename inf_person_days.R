...
results_df %>% 
  ...
  mutate(inf_period=ifelse(is.infinite(inf_start) & is.infinite(inf_end),0, inf_end-inf_start),
         test_num=case_when(stringency==1~0,
                            stringency==2~1,
  ...
  group_by(ind_idx,stringency,assay,test_exit_self_iso,quar_dur,n_tests,test_to_tracing,iso_dur) %>% 
  ...
  select(-data) %>% 
  pivot_wider(values_from=c(`2.5%`:`97.5%`),names_from = name,names_glue = "{name}_{.value}",names_sort = F) %>% 
  mutate(across(c(`tests_used_50%`,`tests_used_2.5%`,`tests_used_97.5%`),function(x){x+uninf_test_num})) %>%
  ...
  relocate(
         ...
         `tests_used_50%`,`tests_used_2.5%`,`tests_used_97.5%`,everything()) %>% write.csv("results/release_results.csv")
...
results_df %>% 
  filter(!is.infinite(inf_start) & !is.infinite(inf_end),
         !multiple_tests,
         test_to_tracing==3) %>% 
  mutate(inf_period=inf_end-inf_start) %>%
  group_by(ind_idx,stringency,assay,test_exit_self_iso,quar_dur,n_tests,test_to_tracing,iso_dur) %>% 
  summarise(all=sum(inf_period),
            in_quar_iso=sum(max_overlap),
            prop=in_quar_iso/all
            ) %>% 
  group_by(stringency,assay,quar_dur,test_exit_self_iso,n_tests,test_to_tracing,iso_dur) %>% 
  nest() %>%
  mutate(Q    = map(.x=data,
                    ~quantile(.$prop,
  ...
  geom_linerange(aes(ymin = `2.5%`,
                     ymax = `97.5%`,
                     colour=factor(iso_dur)),position=position_dodge(width=0.5),size=1.5,alpha=0.3) +
  geom_linerange(aes(ymin = `25%`,
                     ymax = `75%`,
                     colour=factor(iso_dur)),position=position_dodge(width=0.5),size=1.5,alpha=0.5)+
  geom_point(aes(y = `50%`,colour=factor(iso_dur)),
             #pch="-",
             size=1.5,
